...
def authenticate():
    """Login to EarthData via environment variables (CI) or interactive."""
...
def extract_photon_data(h5_path: str) -> pd.DataFrame:
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        # geophys_corr/geoid: geoid height per segment
        ...
        seg_delta_time = seg_grp["delta_time"][:]
...
def main():
    ...
    for fpath in files:
        if str(fpath).endswith(".h5"):
            ...
            print(f"\nSaved all photons to {csv_path}")
            print(f"File size: {csv_path.stat().st_size / 1024:.0f} KB")

            # Filter & save subsurface (multiple scattering) photons
            ...
            if len(df_sub) > 0:
                ...
                print("No subsurface signal photons found in this granule.")
